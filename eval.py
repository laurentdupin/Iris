...
def parse_args():
    ...
    parser.add_argument(
        ...
        default=[499, 999],
        nargs="+",
        help="The timesteps for the training, default is [499, 999] for two-stage prediction."
    )
    parser.add_argument(
        "--task_name",
        type=str,
    ...
    parser.add_argument(
        "--eval_first_stage",
        action="store_true",
        help="When enabled, evaluate the first-stage output instead of the second-stage output."
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Maximum processing resolution. 0 for using input image resolution. Default: 768.",
    )
    parser.add_argument(
...
def main():
    ...
    def gen_depth(rgb_in, pipe, prompt="", num_inference_steps=1):
        ...
        with autocast_ctx:
            ...
            if args.eval_first_stage:
                # Get the first-stage output
                final_output, intermediate_output = pipe(
                    rgb_in=rgb_input,
                    prompt=prompt,
                    ...
                    return_intermediates=True,  # Return the first-stage output
                )
                logging.info("Evaluating first-stage output")
                pred_depth = intermediate_output.images[0]
            else:
                # Get the second-stage output (default behavior)
                pred_depth = pipe(
                    rgb_in=rgb_input,
                    prompt=prompt,
                    num_inference_steps=num_inference_steps,
                ...
                logging.info("Evaluating second-stage output (default)")
    ...
    def gen_normal(img, pipe, prompt="", num_inference_steps=1):
        ...
        with autocast_ctx:
            ...
            if args.eval_first_stage:
                ...
                logging.info("Evaluating first-stage output")
                pred_normal = intermediate_output.images[0]  # [0,1], (3,h,w)
            else:
                # Get the second-stage output (default behavior)
                pred_normal = pipe(
                    rgb_in=img,  # [-1,1]
    ...
    with torch.no_grad():
        if args.task_name == 'depth':
            ...
            for dataset_name, config_path in test_depth_dataset_configs.items():
                stage_suffix = "_stage1" if args.eval_first_stage else "_stage2"
                eval_dir = os.path.join(args.output_dir, args.task_name + stage_suffix, dataset_name)
                ...
                metric_tracker = evaluation_depth(
                    eval_dir,
                    test_dataset_config,
                    test_data_dir,
                    eval_mode="generate_prediction",
                    gen_prediction=gen_depth,
                    pipeline=pipeline,
                ...
                print(
                    dataset_name, ',',
                    'abs_relative_difference: ', metric_tracker.result()['abs_relative_difference'],
                    'delta1_acc: ', metric_tracker.result()['delta1_acc']
                )
            ...
            stage_suffix = "_stage1" if args.eval_first_stage else "_stage2"
            eval_dir = os.path.join(args.output_dir, args.task_name + stage_suffix)
            evaluation_normal(
                eval_dir,
                test_data_dir,
                dataset_split_path,
                eval_mode="generate_prediction",
                gen_prediction=gen_normal,
                pipeline=pipeline,
        ...
        stage_text = "first stage" if args.eval_first_stage else "second stage"
        print(f'==> Evaluation of {stage_text} is done. \n==> Results saved to:', args.output_dir)
