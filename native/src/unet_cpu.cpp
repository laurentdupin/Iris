...
ImageTensor concatenate(
    ...
    if (left.height != right.height || left.width != right.width) {
        throw std::runtime_error("Iris skip shape mismatch");
    }
    ImageTensor output{
...
ImageTensor unet_resnet(
    ...
    if (time_hidden.dimensions != hidden.channels) {
        throw std::runtime_error("Iris time projection shape mismatch");
    }
...
ImageTensor unet_predict(
    ...
    if ((expected_channels != 4 && expected_channels != 8) ||
        ...
        throw std::invalid_argument("invalid Iris UNet input");
    }
    TokenTensor time = embedding_mlp(
        model, timestep_embedding(timestep), "time_embedding");
    TokenTensor labels{1, 4, class_labels};
    TokenTensor class_embedding =
        embedding_mlp(model, labels, "class_embedding");
    if (time.values.size() != class_embedding.values.size()) {
        throw std::runtime_error("Iris class embedding mismatch");
    }
    for (std::size_t i = 0; i < time.values.size(); ++i) {
        time.values[i] += class_embedding.values[i];
    }
    ...
    for (std::uint32_t block = 0; block < 4; ++block) {
        for (std::uint32_t layer = 0; layer < 3; ++layer) {
            if (skips.empty()) {
                throw std::runtime_error("Iris skip stack underflow");
            }
        ...
        if (block != 3) {
            if (skips.empty()) {
                throw std::runtime_error(
                    "Iris skip stack lacks upsample target");
            }
    ...
    if (!skips.empty()) {
        throw std::runtime_error("Iris skip stack was not consumed");
    }
    group_norm(
