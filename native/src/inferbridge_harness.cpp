...
struct ibrh_model {
    ...
    std::shared_ptr<IrisGpuWorker> gpu_worker;
...
struct ibrh_job {
    ...
    std::shared_ptr<iris_native::ExternalJob> gpu_job;
    std::shared_ptr<IrisGpuAdmission> gpu_admission;
    std::weak_ptr<IrisGpuWorker> gpu_worker;
...
constexpr char kHarnessId[] = "inferbridge.iris.native";
...
bool device_index_for_luid(uint64_t luid, int32_t& device_index) {
    ...
    for (int32_t index = 0; index < 32; ++index) {
        try {
            const auto capabilities = iris_native::probe_external_gpu(index);
...
ibrh_result status_result(int status) {
    switch (status) {
        case IRIS_OK: return IBRH_OK;
        case IRIS_INVALID_ARGUMENT:
            return IBRH_ERROR_INVALID_ARGUMENT;
        case IRIS_MODEL_ERROR:
        case IRIS_RUNTIME_ERROR:
        default:
...
class IrisGpuWorker {
public:
    explicit IrisGpuWorker(std::shared_ptr<iris_native::ExternalGpu> gpu)
    ...
    void enqueue(ibrh_job* job) {
        retain_job(job);
        {
            ...
            if (stopping_) {
                release_job(job);
                throw std::runtime_error("Iris GPU worker is stopping");
            }
    ...
    void retire(std::shared_ptr<iris_native::ExternalJob> job) {
        {
            std::lock_guard<std::mutex> lock(mutex_);
...
private:
    void run() noexcept {
        for (;;) {
            ...
            bool stopping = false;
            std::deque<std::shared_ptr<iris_native::ExternalJob>> retired;
            {
            ...
            try {
                ...
                std::fprintf(stderr, "Iris GPU submission failed: %s\n",
                    error.what());
                {
                    std::lock_guard<std::mutex> lock(job->gpu_mutex);
    ...
    std::shared_ptr<iris_native::ExternalGpu> gpu_;
    std::mutex mutex_;
    std::condition_variable condition_;
    std::deque<ibrh_job*> queue_;
    std::deque<std::shared_ptr<iris_native::ExternalJob>> retired_;
...
#if defined(IRIS_WITH_VULKAN) && defined(_WIN32)
    try {
        if (iris_native::probe_external_gpu(0u).available) {
...
#elif defined(IRIS_WITH_METAL) && defined(__APPLE__)
    capabilities->flags |= IBRH_CAP_ASYNC_SUBMIT |
        IBRH_CAP_CANCELLATION | IBRH_CAP_GPU_RESOURCES |
    ...
    if (json_uint64(device, "index", index)) {
        if (index > static_cast<uint64_t>(std::numeric_limits<int32_t>::max())) {
            ...
            return fail(
                ...
                "Iris requested device index is out of range");
        }
    ...
    if (json_string(device, "luid", luid_text) && !luid_text.empty()) {
        ...
        if (!parse_luid(luid_text, luid) ||
            ...
            return fail(nullptr, IBRH_ERROR_UNSUPPORTED_CAPABILITY,
                        "Iris could not match the requested GPU LUID");
        }
...
ibrh_result IBRH_CALL model_load(
    ...
    *output = nullptr;
    ...
    if (!json_string(parameters, "PromptCache", prompt_cache) ||
        ...
        return fail(
            runtime, IBRH_ERROR_INVALID_ARGUMENT,
            "Iris PromptCache path is required");
    }
    ...
    if (runtime->adapter_luid != 0u && !runtime->force_host_transfers) {
        try {
            model->external_gpu = iris_native::create_external_gpu(
                path, prompt_cache,
            ...
            if (!capabilities.available ||
                ...
                throw std::runtime_error(
                    "Iris loaded on a GPU other than the requested LUID");
            model->gpu_worker = std::make_shared<IrisGpuWorker>(
                model->external_gpu);
        } catch (const std::exception& error) {
            delete model;
...
#endif
    {
        const int status = iris_create_vulkan(
            path.c_str(), prompt_cache.c_str(),
            static_cast<uint32_t>(runtime->vulkan_device_index),
            &model->context);
        if (status != IRIS_OK) {
            const std::string message =
                std::string("Iris model load failed: ") + iris_last_error();
            delete model;
            return fail(runtime, status_result(status), message);
        }
#if defined(IRIS_WITH_METAL) && defined(__APPLE__)
        model->external_gpu = iris_native::create_metal_external_gpu(
...
#endif
    if (model->context) iris_destroy(model->context);
    delete model;
}
    ...
    *output = nullptr;
    ...
    if (request->input_count != 1u || !request->inputs ||
        ...
        return fail(model->runtime, IBRH_ERROR_INVALID_ARGUMENT,
                    "Iris requires one input and one Core-owned output");
    ...
    if(iris_inferbridge_image_shape(input.width,input.height,&planned_width,&planned_height)!=IRIS_OK)return IBRH_ERROR_INVALID_ARGUMENT;
    if (destination.width != planned_width || destination.height != planned_height ||
        ...
        return fail(model->runtime, IBRH_ERROR_INVALID_ARGUMENT,
                    "Iris Core-owned output does not match the output plan");
    ...
    if (parameters.find("\"Seed\"") != std::string::npos &&
        ...
        return fail(model->runtime, IBRH_ERROR_INVALID_ARGUMENT,
                    "Iris Seed must be an unsigned integer");
    ...
    if (input.domain == gpu_domain) {
        ...
        if (!model->gpu_worker)
            return fail(model->runtime, IBRH_ERROR_UNSUPPORTED_CAPABILITY,
                        "Iris GPU model was not loaded for external input");
        if (input.kind != IBRH_RESOURCE_KIND_IMAGE_2D ||
            ...
            return fail(model->runtime, IBRH_ERROR_INVALID_ARGUMENT,
                        "Iris external transfer bindings are invalid");
        ...
        try {
            job->gpu_admission = std::make_shared<IrisGpuAdmission>(
    ...
    if (input.domain != IBRH_RESOURCE_DOMAIN_HOST ||
        ...
        return fail(model->runtime, IBRH_ERROR_UNSUPPORTED_CAPABILITY,
                    "Iris host transfer bindings are invalid");
    auto* job = new (std::nothrow) ibrh_job();
        ...
        static_cast<uintptr_t>(destination.native_handle) + destination.byte_offset);
    {
        ...
        const int status = iris_infer_bgra8_f32(
            model->context, bgra, input.width, input.height,
            input.row_stride_bytes, seed, depth);
        if (status != IRIS_OK) {
            const std::string message =
                std::string("Iris inference failed: ") + iris_last_error();
            delete job;
    ...
    *status = {};
    ...
    if (job->gpu_admission) {
        ...
        if (!gpu_job) status->state = job->gpu_state.load();
            ...
            case iris_native::ExternalJobState::running:
                status->state = IBRH_JOB_RUNNING; break;
            case iris_native::ExternalJobState::complete:
...
#if defined(__linux__) && !defined(__ANDROID__) && defined(IRIS_WITH_VULKAN)
...
struct LinuxCaptureHooks {
    static ibr_linux_capture_capabilities capabilities(ibrh_model* model) {
        std::lock_guard<std::mutex> lock(model->submit_mutex);
        return iris_linux_capture_capabilities(model->context);
    }
