...
public:
    ...
    void build() {
        ...
        MPSGraphTensor* intermediate = unet(sample, 999u);
        MPSGraphTensor* prediction = unet(intermediate, 499u);
...
private:
    ...
    MPSGraphTensor* time_embedding(std::uint32_t timestep_value) {
    ...
    MPSGraphTensor* unet(
        MPSGraphTensor* sample, std::uint32_t timestep_value) {
        MPSGraphTensor* time = time_embedding(timestep_value);
...
public:
    Impl(const ModelBundle& model, const TokenTensor& prompt)
        ...
        if (precision == inferbridge::native::Precision::int8)
            throw std::invalid_argument("Iris Metal does not support INT8 yet");
        ...
        if (queue_ == nil || graph_device_ == nil)
            throw std::runtime_error("could not initialize Iris Metal");
        inferbridge::native_harness::metal::label_queue(queue_, "Iris");
        ...
        @autoreleasepool {
            ...
            auto auxiliary = auxiliary_pool_->acquire({
                ...
                {{1, 4, height / 8, width / 8}, MPSDataTypeFloat32,
                    sizeof(float), MTLResourceStorageModeShared,
                    "Posterior Noise"}});
            std::memcpy(auxiliary->buffer(0).contents, initial.data(),
                initial.size() * sizeof(float));
            std::memcpy(auxiliary->buffer(1).contents, posterior.data(),
                posterior.size() * sizeof(float));
            prepared.retained_resources.push_back(auxiliary);
            NSArray<MPSGraphTensorData*>* inputs = @[prepared.input_data,
                auxiliary->data(0), auxiliary->data(1)];
            ...
            if (results.count != 1u)
                throw std::runtime_error("Iris Metal output binding failed");
    ...
    ImageTensor infer(
        ...
        @autoreleasepool {
            ...
            if (results.count != 1)
                throw std::runtime_error("Iris Metal returned incomplete output");
...
private:
    const Plan& get_plan(std::uint32_t width, std::uint32_t height) {
        ...
        if (executable == nil)
            throw std::runtime_error("failed to compile Iris Metal graph");
    ...
    const Plan& get_presentation_plan(
        ...
        if (executable == nil)
            throw std::runtime_error("failed to compile Iris Metal presentation graph");
        executable.options = MPSGraphOptionsSynchronizeResults;
    ...
    NSURL* cache_url(const PlanKey& key) const {
        if (@available(macOS 14.0, *)) {
            ...
            NSString* directory = [[NSString
                ...
                stringByAppendingPathComponent:@"IrisMetalGraphCache-v1"];
