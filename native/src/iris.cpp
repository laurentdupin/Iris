...
struct iris_context {
    std::unique_ptr<iris_native::ModelBundle> model;
    iris_native::TokenTensor prompt;
#if defined(IRIS_WITH_METAL)
    std::unique_ptr<iris_native::MetalExecutor> metal;
#endif
#if defined(IRIS_WITH_VULKAN)
    std::unique_ptr<iris_native::VulkanContext> vulkan;
    std::unique_ptr<iris_native::GpuModel> gpu_unet;
    std::unique_ptr<iris_native::GpuModel> gpu_vae;
    std::unique_ptr<iris_native::VulkanOperators> operators;
#endif
};
...
public:
    explicit MetalContextExternalGpu(iris_context* context) : context_(context) {}
    ...
    std::shared_ptr<ExternalJob> submit_texture(
        ...
        if (!context_ || !context_->metal)
            throw std::invalid_argument("Iris Metal context is unavailable");
...
std::shared_ptr<ExternalGpu> create_metal_external_gpu(
    ...
    (void)context;
    (void)cache_path;
    throw std::invalid_argument("Iris was built without Metal");
#endif
}
}
...
iris_native::ImageTensor infer(
    ...
    if (width < 8 || height < 8 || latent_width == 0 || latent_height == 0) {
        throw std::invalid_argument("Iris input dimensions are too small");
    }
    iris_native::ImageTensor image{
        3, height, width,
    ...
    const iris_native::Posterior posterior =
        iris_native::vae_encode(context.model->vae(), image);
    const std::uint64_t latent_elements =
        std::uint64_t(4) * latent_height * latent_width;
    if (posterior.mean.values.size() != latent_elements) {
        throw std::runtime_error("Iris latent dimensions are inconsistent");
    }
    iris_native::ImageTensor rgb_latent{
    ...
    iris_native::ImageTensor sample{
    ...
    if (input_channels == 8) {
        ...
        throw std::runtime_error("unsupported Iris UNet input channels");
    }
    const std::vector<float> task = {
        std::sin(1.0f), 0.0f, std::cos(1.0f), 1.0f};
    iris_native::ImageTensor intermediate = iris_native::unet_predict(
        context.model->unet(), sample, 999, context.prompt, task);
    iris_native::ImageTensor prediction = iris_native::unet_predict(
        context.model->unet(), intermediate, 499, context.prompt, task);
    for (float& value : prediction.values) {
        value /= 0.18215f;
    }
    return iris_native::vae_decode(context.model->vae(), prediction);
}

void output_depth(
    const iris_native::ImageTensor& decoded,
...
int iris_get_transfer_counters(
    uint64_t* upload_bytes, uint64_t* download_bytes) {
    if (!upload_bytes || !download_bytes)
        return fail(IRIS_INVALID_ARGUMENT, "invalid transfer counter output");
#if defined(IRIS_WITH_VULKAN)
    iris_native::global_transfer_counters(*upload_bytes, *download_bytes);
#else
    *upload_bytes = 0u;
    *download_bytes = 0u;
#endif
...
std::uint32_t iris_native_abi_version(void) {
    return IRIS_NATIVE_ABI_VERSION;
}

const char* iris_last_error(void) {
...
int iris_create(
    ...
    if (!snapshot_root || !prompt_cache || !output) {
        return fail(IRIS_INVALID_ARGUMENT, "invalid Iris create argument");
    }
    *output = nullptr;
    try {
        auto context = std::make_unique<iris_context>();
        context->model =
            std::make_unique<iris_native::ModelBundle>(
                snapshot_root, false);
        const std::uint32_t input_channels = static_cast<std::uint32_t>(
            context->model->unet().tensor(
                "conv_in.weight").dimensions[1]);
        context->prompt = iris_native::load_empty_prompt_cache(
            prompt_cache, input_channels);
        *output = context.release();
        last_error.clear();
        return IRIS_OK;
    } catch (const std::exception& error) {
        return fail(IRIS_MODEL_ERROR, error);
    }
}

int iris_create_vulkan(
    const char* snapshot_root,
    const char* prompt_cache,
    ...
    *output = nullptr;
    ...
    try {
        auto context = std::make_unique<iris_context>();
        context->model = std::make_unique<iris_native::ModelBundle>(
            snapshot_root, false);
        const std::uint32_t input_channels = static_cast<std::uint32_t>(
            context->model->unet().tensor("conv_in.weight").dimensions[1]);
        context->prompt = iris_native::load_empty_prompt_cache(
            prompt_cache, input_channels);
        context->metal = std::make_unique<iris_native::MetalExecutor>(
...
#elif !defined(IRIS_WITH_VULKAN)
    (void)device_index;
    return fail(IRIS_RUNTIME_ERROR, "this DLL was built without Vulkan");
#else
    try {
        auto context = std::make_unique<iris_context>();
        context->model =
        ...
        context->prompt = iris_native::load_empty_prompt_cache(
            prompt_cache, input_channels);
        context->vulkan =
            std::make_unique<iris_native::VulkanContext>(device_index);
        context->gpu_unet = std::make_unique<iris_native::GpuModel>(
            context->model->unet(), *context->vulkan);
        context->gpu_vae = std::make_unique<iris_native::GpuModel>(
            context->model->vae(), *context->vulkan);
        context->operators =
            std::make_unique<iris_native::VulkanOperators>(
                *context->vulkan);
        *output = context.release();
        last_error.clear();
        return IRIS_OK;
...
void iris_destroy(iris_context* context) {
    delete context;
}

int iris_infer_rgb_f32_with_noise(
    iris_context* context,
    const float* rgb,
    std::uint32_t width,
    std::uint32_t height,
    ...
    if (!context || !rgb || !initial_noise || !posterior_noise || !depth) {
        return fail(IRIS_INVALID_ARGUMENT, "invalid Iris inference argument");
    }
    try {
#if defined(IRIS_WITH_METAL)
        if (context->metal) {
...
#if defined(IRIS_WITH_VULKAN)
        if (context->vulkan) {
            iris_native::VulkanBuffer output =
                iris_native::iris_infer_gpu(
        ...
        return fail(IRIS_RUNTIME_ERROR, error);
    }
}

int iris_infer_rgb_f32(
    iris_context* context,
    const float* rgb,
    std::uint32_t width,
    std::uint32_t height,
    ...
    return iris_infer_rgb_f32_with_noise(
        context, rgb, width, height,
        initial.data(), posterior.data(), depth);
}

int iris_inferbridge_image_shape(
    std::uint32_t source_width,
    std::uint32_t source_height,
    std::uint32_t* processing_width,
    std::uint32_t* processing_height) {
    if (!processing_width || !processing_height) {
        return fail(IRIS_INVALID_ARGUMENT, "invalid Iris shape argument");
    }
    try {
        inferbridge_shape(
        ...
        return fail(IRIS_INVALID_ARGUMENT, error);
    }
}

int iris_infer_bgra8_f32_with_noise(
    iris_context* context,
    const std::uint8_t* bgra,
    std::uint32_t width,
    std::uint32_t height,
    ...
    if (!context || !bgra || !initial_noise || !posterior_noise || !depth ||
        ...
        return fail(
            IRIS_INVALID_ARGUMENT, "invalid Iris BGRA inference argument");
    }
    try {
        ...
        const int result = iris_infer_rgb_f32_with_noise(
...
int iris_infer_bgra8_f32(
    ...
    std::uint32_t processing_width = 0;
    std::uint32_t processing_height = 0;
    const int shape_result = iris_inferbridge_image_shape(
        width, height, &processing_width, &processing_height);
    if (shape_result != IRIS_OK) {
        return shape_result;
    }
    const std::uint64_t count =
    ...
    return iris_infer_bgra8_f32_with_noise(
