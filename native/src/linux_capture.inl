...
iris_linux_capture_capabilities(iris_context *context) {
...
void iris_infer_linux_capture(
    ...
    float *output) {
  if (!context->vulkan)
    throw std::runtime_error("iris Vulkan context unavailable");
  ...
  auto first = noise();
  iris_native::IrisGpuGraph graph(vk, *context->gpu_unet, *context->gpu_vae,
