...
private:
    ...
    void metadata() {
        ...
        for (;;) {
            ...
            // Hugging Face files conventionally carry {"format":"pt"}.
            // Other string pairs are retained as shared-tensor aliases.
            if (alias != "format") {
    ...
    ParsedTensor tensor(const std::string& name) {
        ...
        for (;;) {
            ...
            if (field == "dtype") {
                ...
                whitespace();
                if (consume(']')) {
                    fail("Iris scalar tensors are unsupported");
                }
                for (;;) {
                    if (result.rank >= result.dimensions.size()) {
                        fail("Iris tensor rank exceeds four");
                    }
