...
const TensorView& vector_tensor(
    ...
    if (tensor.rank != 1 || tensor.dimensions[0] != count) {
        throw std::runtime_error("Iris vector shape mismatch: " + name);
    }
    return tensor;
}
...
ImageTensor conv2d(
    ...
    if (weight.rank != 4 ||
        ...
        throw std::runtime_error(
            "Iris convolution shape mismatch: " + weight_name);
    }
    ...
    if (input.height + pad_before + pad_after < kernel ||
        ...
        throw std::runtime_error("Iris convolution input is too small");
    }
    ImageTensor output{
        output_channels,
...
void group_norm(
    ...
    if (tensor.channels % groups != 0) {
        throw std::runtime_error("Iris group norm channel mismatch");
    }
...
void add_in_place(ImageTensor& destination, const ImageTensor& source) {
    if (destination.channels != source.channels ||
        ...
        throw std::runtime_error("Iris residual shape mismatch");
    }
    parallel_for(
...
ImageTensor spatial_attention(
    ...
    const auto project = [&](const char* suffix, std::vector<float>& out) {
        ...
        if (weight.rank != 2 ||
            ...
            throw std::runtime_error(
                "Iris attention projection shape mismatch: " + base);
        }
        parallel_for(tokens, [&](std::uint32_t begin, std::uint32_t end) {
            for (std::uint32_t token = begin; token < end; ++token) {
                for (std::uint32_t oc = 0; oc < channels; ++oc) {
                    ...
                    const float* row =
                        weight.data + std::uint64_t(oc) * channels;
    ...
    parallel_for(tokens, [&](std::uint32_t begin, std::uint32_t end) {
        for (std::uint32_t token = begin; token < end; ++token) {
            for (std::uint32_t oc = 0; oc < channels; ++oc) {
                ...
                const float* row =
                    output_weight.data + std::uint64_t(oc) * channels;
                for (std::uint32_t ic = 0; ic < channels; ++ic) {
                    sum += attended[
