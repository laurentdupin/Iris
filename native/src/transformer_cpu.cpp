...
void add(TokenTensor& destination, const TokenTensor& source) {
    if (destination.tokens != source.tokens ||
        ...
        throw std::runtime_error("Iris token residual shape mismatch");
    }
    parallel_for(
...
TokenTensor layer_norm(
    ...
    if (weight.rank != 1 ||
        ...
        throw std::runtime_error("Iris layer norm shape mismatch");
    }
...
TokenTensor attention(
    ...
    if (q.dimensions != k.dimensions ||
        ...
        throw std::runtime_error("Iris attention head mismatch");
    }
...
TokenTensor feed_forward(
    ...
    if (projected.dimensions % 2 != 0) {
        throw std::runtime_error("Iris GEGLU dimension mismatch");
    }
...
TokenTensor linear(
    ...
    if (weight.rank != 2 ||
        ...
        throw std::runtime_error(
            "Iris linear shape mismatch: " + weight_name);
    }
    ...
    if (!bias_name.empty()) {
        ...
        if (bias_view.rank != 1 ||
            ...
            throw std::runtime_error(
                "Iris linear bias mismatch: " + bias_name);
        }
        bias = bias_view.data;
    }
    TokenTensor output{
    ...
    parallel_for(input.tokens, [&](std::uint32_t begin, std::uint32_t end) {
        for (std::uint32_t token = begin; token < end; ++token) {
            ...
            for (std::uint32_t out = 0; out < output_dimensions; ++out) {
                ...
                const float* row =
                    weight.data + std::uint64_t(out) * input.dimensions;
                for (std::uint32_t in = 0; in < input.dimensions; ++in) {
                    sum += source[in] * row[in];
                }
                destination[out] = sum;
            }
        }
