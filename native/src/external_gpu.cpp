...
void validate_input(
    ...
    check_hresult(device->OpenSharedHandle(
        ...
        IID_PPV_ARGS(&resource)), "OpenSharedHandle(Iris input)");
    ...
    if (description.Dimension != D3D12_RESOURCE_DIMENSION_TEXTURE2D ||
        ...
        throw std::invalid_argument("shared Iris input texture is invalid");
}
...
void validate_output(
    ...
    check_hresult(device->OpenSharedHandle(
        ...
        IID_PPV_ARGS(&resource)), "OpenSharedHandle(Iris output)");
    const D3D12_RESOURCE_DESC description = resource->GetDesc();
    if (description.Dimension != D3D12_RESOURCE_DIMENSION_TEXTURE2D ||
        ...
        throw std::invalid_argument("shared Iris output texture is invalid");
}
...
public:
    ExternalGpuImpl(
        const std::string& root, const std::string& prompt_cache,
        std::uint32_t device_index)
        : model_(root, false), context_(device_index),
          ...
          graph_(context_, unet_, vae_, operators_, prompt_)
#if defined(_WIN32)
    ...
    std::shared_ptr<ExternalJob> submit_texture(
...
#if !defined(_WIN32)
        (void)request;
        throw std::runtime_error("Iris D3D12 interop is unavailable");
#else
        if (!capabilities().available)
            throw std::runtime_error("Iris external GPU path is unavailable");
        if (!request.shared_texture_handle || !request.wait_fence_handle ||
            ...
            throw std::invalid_argument("invalid Iris external GPU request");
        try {
            ...
            if (!request.output_width ||
                ...
                throw std::invalid_argument("Iris output dimensions do not match its plan");
            ...
            try {
                ...
                throw std::runtime_error(
                    std::string("Iris preprocess/noise stage failed: ") +
                    error.what());
            }
            ...
            try {
                ...
                throw std::runtime_error(
                    std::string("Iris bounded diffusion/VAE stage failed: ") +
                    error.what());
            }
            ...
            try {
                ...
                throw std::runtime_error(
                    std::string("Iris final R32 output stage failed: ") +
                    error.what());
            }
...
std::shared_ptr<ExternalGpu> create_external_gpu(
    const std::string& snapshot_root, const std::string& prompt_cache,
    std::uint32_t device_index) {
    return std::make_shared<ExternalGpuImpl>(
        snapshot_root, prompt_cache, device_index);
}
