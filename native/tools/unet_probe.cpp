...
int main(int argc, char** argv) {
    if (argc != 3) {
        std::cerr << "usage: iris_unet_probe snapshot-root fixture-dir\n";
        return 2;
    }
    try {
        ...
        iris_native::ImageTensor sample{8, 8, 8, {}};
        ...
        const iris_native::TokenTensor prompt{
            77, 1024, load(root + "/prompt.bin", 77 * 1024)};
        const std::vector<float> task = load(root + "/task.bin", 4);
        const iris_native::ImageTensor prediction =
            iris_native::unet_predict(
