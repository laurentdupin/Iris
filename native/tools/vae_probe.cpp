...
int main(int argc, char** argv) {
    if (argc != 3) {
        std::cerr << "usage: iris_vae_probe snapshot-root fixture-dir\n";
        return 2;
    }
    try {
        const iris_native::ModelBundle bundle(argv[1]);
        const std::string fixture = argv[2];
        iris_native::ImageTensor rgb{
            3, 64, 64, load((fixture + "/rgb.bin").c_str(), 3 * 64 * 64)};
        const iris_native::Posterior posterior =
            iris_native::vae_encode(bundle.vae(), rgb);
        compare(
        ...
        iris_native::ImageTensor latent{
            4, 8, 8,
        ...
        const iris_native::ImageTensor decoded =
            iris_native::vae_decode(bundle.vae(), latent);
        compare(
