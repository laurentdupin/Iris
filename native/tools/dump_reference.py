"""Generate deterministic Python CPU fixtures for the native Iris port."""
...
def main() -> None:
    ...
    if unet.config.in_channels == 8:
        ...
        raise RuntimeError(
            f"unsupported Iris UNet input channels: {unet.config.in_channels}"
        )
    intermediate = unet(
        unet_input,
        torch.tensor(999),
    ...
    save(output / "unet_intermediate.bin", intermediate)
