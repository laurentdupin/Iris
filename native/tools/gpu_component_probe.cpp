...
int main(int argc, char** argv) {
    ...
    try {
        ...
        iris_native::SafeTensors unet(
            (snapshot / "unet/diffusion_pytorch_model.safetensors").string());
        iris_native::SafeTensors vae(
            (snapshot / "vae/diffusion_pytorch_model.safetensors").string());
        iris_native::VulkanContext context(std::stoul(argv[4]));
        iris_native::VulkanOperators operators(context);
        {
        ...
        iris_native::GpuModel gpu_vae(vae, context);
        {
            iris_native::ImageTensor input{
            ...
            iris_native::TokenTensor normalized_tokens{
            ...
            const auto q_reference = iris_native::linear(
            ...
            const auto k_reference = iris_native::linear(
                vae, normalized_tokens, prefix + ".to_k.weight",
                prefix + ".to_k.bias");
            const auto v_reference = iris_native::linear(
            ...
            iris_native::TokenTensor attended_tokens{
                64, 512, attended_reference};
            const auto projected_reference = iris_native::linear(
            ...
            const auto reference =
                iris_native::spatial_attention(vae, input, prefix);
            auto output = iris_native::iris_spatial_attention_gpu(
        ...
        }
        {
            ...
            auto output = iris_native::iris_vae_encode_gpu(
        ...
        iris_native::GpuModel gpu_unet(unet, context);
        {
            ...
            auto prompt = iris_native::load_empty_prompt_cache(
                argv[2], input_channels);
            auto output = iris_native::iris_unet_gpu(
        ...
        }
        {
            ...
            auto output = iris_native::iris_vae_decode_gpu(
