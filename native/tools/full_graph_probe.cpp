...
int main(int argc, char** argv) {
    ...
    const int create_status = argc >= 5
        ...
        : iris_create(argv[1], argv[2], &context);
    if (create_status != IRIS_OK) {
        std::cerr << iris_last_error() << "\n";
        return 1;
    }
    try {
        ...
        for (std::uint32_t iteration = 0; iteration < iterations; ++iteration) {
            ...
            const int status = iris_infer_rgb_f32_with_noise(
                context, rgb.data(), size, size,
                initial.data(), posterior.data(), depth.data());
            samples.push_back(std::chrono::duration<double, std::milli>(
                std::chrono::steady_clock::now() - start).count());
            if (status != IRIS_OK) {
                throw std::runtime_error(iris_last_error());
            }
        }
