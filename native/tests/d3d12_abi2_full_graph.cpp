...
int main() try {
    const char* model_environment = std::getenv("IRIS_MODEL");
    ...
    if ((capabilities.flags & required) != required)
        throw std::runtime_error("Iris ABI2 GPU capability is incomplete");
    ...
    const char* prompt_environment = std::getenv("IRIS_PROMPT_CACHE");
    ...
    check_counter(iris_get_transfer_counters(
    ...
    if (submit_ms > 5.0) throw std::runtime_error("Iris submit exceeded 5 ms");
    ...
    if (status.state != IBRH_JOB_COMPLETE || status.source_frame_id != 7007u)
        throw std::runtime_error("Iris ABI2 job did not complete with correlation");
    ...
    if (upload_after != upload_before || download_after != download_before)
        throw std::runtime_error(
            "Iris external path staged tensor bytes through host");
    ...
    if (finite_count < gpu.size() / 2u)
        throw std::runtime_error("Iris external output has too few valid mask pixels");
    ...
    if (!(maximum > minimum))
        throw std::runtime_error(
            "Iris external output is not varying: min=" +
    ...
    for (std::uint64_t frame = 7008u;
        ...
        if (status.state != IBRH_JOB_COMPLETE || status.source_frame_id != frame)
            throw std::runtime_error("Iris stress correlation failed");
        wait_fence(selected.device.Get(), ready);
    }
    api.job_release(job);
