...
int main() {
    const char* path = std::getenv("IRIS_SNAPSHOT");
    const char* prompt = std::getenv("IRIS_PROMPT_CACHE");
    const char* variant = std::getenv("IRIS_VARIANT");
    if (path == nullptr || *path == '\0' ||
        prompt == nullptr || *prompt == '\0') {
        std::cout <<
            "IRIS_SNAPSHOT/IRIS_PROMPT_CACHE is not set; skipping\n";
        return 77;
    }
    ibrh_api api{};
    if (!check(
